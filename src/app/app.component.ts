...
export class AppComponent implements OnInit {
  ...
  actionTable: Array<PoTableAction> = [
    {
      label: 'Deletar',
      action: () => this.notification.success('Deletado com sucesso'),
      icon: 'po-icon po-icon-align-justify',
    },
    {
      label: 'Comprar',
      action: () => this.notification.success('Compra com sucesso'),
  ...
  profileActions: Array<PoToolbarAction> = [
    {
      icon: 'po-icon-user',
      label: this.sair,
      action: () => this.notification.error(this.sair),
    },
    {
      icon: 'po-icon-company',
      label: this.documentacao,
      action: () => this.notification.error(this.documentacao),
    },
  ];

  columnData: Array<PoTableColumn> = [
    { property: 'country', label: 'Pais' },
    { property: 'domains', label: 'Dominio' },
    { property: 'web_pages', label: 'Paginas' },
    { property: 'alpha_two_code', label: 'Código' },
    { property: 'name', label: 'Nome' },
    { property: 'name', label: 'Nome',visible:false, disabled: () => true  },
  ];
  constructor(
  ...
  ngOnInit(): void {
    ...
    this.http
      ...
      .subscribe((response) => {
        this.resposerAPI = response;
      });
  }

  selectedAll() {
    this.notification.warning('Todos os registros selecionados');
  }

  unselectedAll() {
    this.notification.information('Registros não selecionados');
  }

  visibleColumn() {
    this.notification.warning('Visible executado');
  }

  restoreTable() {
    this.notification.warning('Table restaurada');
  }

  lineSelected(event: any){
    this.notification.success(`Clicamos no ${event.country}`)
  }

  sortEvent(){
    this.notification.error("Ordenado")
  }
}
